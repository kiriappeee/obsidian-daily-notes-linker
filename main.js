...
function getAllNotesSorted() {
  ...
  const sortedDates = Object.keys(dailyNotes).sort((a, b) => {
    return (0, import_moment.default)(a, "YYYY-MM-DD").valueOf() - (0, import_moment.default)(b, "YYYY-MM-DD").valueOf();
  });
  return sortedDates.map((date) => dailyNotes[date]);
}
function getNeighbors(sortedNotes, currentNote) {
...
var PREV_LINK_REGEX = /\[\[[^|]*\|<--\s*previous\]\]/i;
var NEXT_LINK_REGEX = /\[\[[^|]*\|next\s*-->\]\]/i;
async function updateBacklink(app, previousFile, newNote) {
  await app.vault.process(previousFile, (data) => {
    return data.replace(NEXT_LINK_REGEX, `[[${newNote.basename}|next -->]]`);
  });
}
async function updateForwardLink(app, nextFile, newNote) {
  await app.vault.process(nextFile, (data) => {
    return data.replace(PREV_LINK_REGEX, `[[${newNote.basename}|<-- previous]]`);
  });
}
async function updateCurrentNoteLinks(app, currentNote, neighbors) {
...
async function linkDailyNote(app, currentNote) {
